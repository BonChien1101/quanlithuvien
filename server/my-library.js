...
router.get('/profile', authenticate, async (req, res) => {
  try {
    ...
    let reader = await Reader.findOne({ where: { userId } });
    // Tự tạo Reader nếu chưa có
    if (!reader) {
      reader = await Reader.create({
        userId,
        name: req.user.username || 'Người dùng',
        quota: 3,
        maxQuota: 3
      });
    }
...
router.get('/current-loans', authenticate, async (req, res) => {
  try {
    ...
    let reader = await Reader.findOne({ where: { userId } });
    if (!reader) {
      reader = await Reader.create({ userId, name: req.user.username || 'Người dùng', quota: 3, maxQuota: 3 });
    }
